...
match(spp[[30]], comadre)
match(spp[[1003]], compadre)
...
TaxaMeta <- data.frame(matrix(NA, ncol=7, nrow=length(mosaic@species)))
rownames(TaxaMeta) <- mosaic@species
colnames(TaxaMeta) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")


for(i in 1:length(mosaic@species)){
